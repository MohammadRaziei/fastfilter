...
template <typename T>
inline std::vector<T> movingRankFilterBind(std::vector<T>& arr, const uint32_t WindowSize, const std::string& kernel){
    std::vector<T> output(arr.size()); 
    if (kernel == "maximum") { 
      filt::RankFilter<T> rankFilterMax(WindowSize, -1); 
      rankFilterMax(output, arr); 
    }
    else if (kernel == "minimum") { 
      filt::RankFilter<T> rankFilterMin(WindowSize, 0); 
      rankFilterMin(output, arr); 
    }
    return arr;
}
...
m.def("rankfilter", &movingRankFilterBind<double>, nb::arg("arr"), nb::arg("WindowSize"), nb::arg("kernel")="maximum");
